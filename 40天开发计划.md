...
| `application.yml` | ✅ 已完成 | 直连 MySQL/Redis，默认连 Redis（已删 local profile） |
| Entity + Mapper（13 实体 / 14 Mapper） | ✅ 已完成 | D2，`@MapperScan` + 单测查库通过 |
| 实体 Jakarta Validation | ✅ 已完成 | `Create`/`Update` 分组 + 对齐表字段约束 |
| 统一响应 `Result<T>` | 🟡 进行中 | 已有基础结构，待统一命名/错误码 |
| 全局异常 + `@Valid` | 🟡 进行中 | `GlobalExceptionHandler` 已处理校验异常 |
| CORS 配置 | ❌ 未开始 | D3 剩余 |
...
**当前阶段**：D3 — 完善框架层（CORS、统一 `Result` 规范），随后 D4 中间件连通。

**计划调整**：D1~D2 已提前完成；D3 实体校验已落地，CORS 与中间件按日历推进即可。
...
│   ├── MesApplication.java
│   ├── common/          # R<T>、异常、枚举、常量
│   ├── config/          # Redis、RocketMQ、Security、MyBatis-Plus、CORS
│   ├── module/
│   │   ├── auth/        # 登录、JWT、用户查询
...
| D3 | 统一响应 `Result<T>`、全局异常、`@Valid` 校验；CORS 配置 | 基础框架 | 实体校验已完成；CORS 待做 |
| D4 | RedisTemplate + Redisson 配置；RocketMQ Producer/Consumer 骨架 | 中间件连通 | 写一个 send/consume 测试 |
| D5 | Spring Security + Auth0 JWT；角色 ROLE_PLANNER/WORKER 等 | 登录 API | 白名单读 `mes.security.permit-urls` |
