...
**当前阶段**：地基 + 鉴权 + ERP 推单 + 工单只读查询已落地；下一步 **Apifox 自测 → D7 前端 → D9 状态机**。

**下一步**：D6/D8 Apifox 自测 → D7 Vue → D9 状态机；RocketMQ **D22 再配**。
...
| D7 | AI：Vue 壳子 + 登录页；Apifox 集合 | `docs/api.md` | **下一步** |

**本周里程碑**：登录 ✅；推单 API ✅（代码）；**Apifox 验证推单 + 重复推单拦截** 待勾。
...
- [ ] 0已下发 → 1已派工 → 2执行中 → 3部分完工 → 4已完工 → 5已关闭 / 6已取消（D9 状态机）
