...
| 工单查询 | 🟡 | D8 分页 + 详情；Apifox 待做 |
| 工单状态机 | ✅ | D9 `WorkOrderStateMachine` + JUnit 7 条 |
| 派工 / 报工 | ❌ | D16+ / D22+ |
| Vue 前端 | ✅ | D7 壳子 + 登录页 + `/admin` / `/terminal` 路由 |

**当前阶段**：后端地基 + 鉴权 + ERP 推单 + 工单查询 + 状态机 + 前端登录壳已落地；下一步 **Apifox 自测 → 前后端登录联调 → D10 ERP 关单**。

**下一步**：D6/D8 Apifox 自测 → 登录联调验证 → D10 关单；RocketMQ **D22 再配**。
...
| 天 | 任务 | 产出 |
|----|------|------|
| D8 | 工单分页 + 详情（含工序列表） | 工单 API | **🟡** 代码完成；Apifox 待做 |
| D9 | **工单状态机** + JUnit 单测 5 条 | 状态机可测 | **✅** 7 态跃迁表 + `resolveAfterReport` + 7 条单测 |
...
**下一步**：D6/D8 Apifox 自测 → 前后端登录联调 → D10 ERP 关单；RocketMQ **D22 再配**。
