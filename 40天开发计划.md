...
> 适用对象：有 Web 开发经验的应届生，40 天内完成可演示、可讲清的 MES 项目。  
...
| `application.yml` | 已完成 | MySQL / Redis / RocketMQ / JWT / 日志 |
| 业务代码 | 未开始 | 从 D3 框架层正式开发 |

**计划调整**：D1~D2 部分任务已提前完成，节省时间并入 D3~D7 做框架与联调；后续日历按「剩余工作」理解即可。
...
| D2 | Entity/Mapper 生成；`@MapperScan`；连接 MySQL 跑通 | 能查 `sys_user` | sql 已有，专注 Java 层 |
| D3 | 统一响应 `R<T>`、全局异常、`@Valid` 校验；CORS 配置 | 基础框架 | 前端联调必备 |
