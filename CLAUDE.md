...
| `mes_db.sql`（表结构 + 种子数据） | ✅ |
| `SecurityConfig`（JWT 白名单预留） | ✅ |
| `docker-compose.yml`（MySQL + Redis） | ✅ |
| Entity / Mapper（13 实体 + 14 Mapper） | ✅ |
| 实体 Jakarta Validation（`Create`/`Update` 分组） | ✅ |
| `Result<T>` + `GlobalExceptionHandler` | 🟡 基础版 |
| CORS 配置 | ❌ |
| Vue 前端 | ❌ |
| RocketMQ Producer/Consumer | ❌ |

**当前开发阶段**：D3 — 完善框架层（CORS、统一响应规范），随后 D4 Redis/MQ 骨架。
...
- 参数校验：实体 `@NotBlank`/`@NotNull` + `Create`/`Update` 分组；Controller 用 `@Validated(Create.class)`
...
1. **D3 剩余**：CORS 配置（可先看 Spring 文档自己写 `WebMvcConfigurer`）
2. **D4**：RedisTemplate / Redisson 配置类 → 连通性自测（AI 只讲配置项含义）
3. **D5**：JWT 登录全流程（**必须手写** Security 过滤器 + 登录 API）
